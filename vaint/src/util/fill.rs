...
// mod platform {
//     use std::{cell::RefCell, collections::HashMap, mem, mem::ManuallyDrop, num::NonZeroU32};
...
//         static GC: ManuallyDrop<RefCell<Option<GraphicsContext>>> = const {
...
//             Self {
//                 context: RefCell::new(
//                     Context::new(unsafe { mem::transmute::<&'_ dyn Window, &'static dyn
// Window>(w) })                         .expect("Failed to create a softbuffer context"),
//                 ),
//                 surfaces: HashMap::new(),
//             }
//         }

//         fn create_surface(&mut self, window: &dyn Window) -> &mut Surface<&'static dyn Window,
// &'static dyn Window> {             self.surfaces.entry(window.id()).or_insert_with(|| {
//                 Surface::new(&self.context.borrow(), unsafe { mem::transmute::<&'_ dyn Window,
// &'static dyn Window>(window) })                     .expect("Failed to create a softbuffer
// surface")             })
//         }

//         fn destroy_surface(&mut self, window: &dyn Window) { self.surfaces.remove(&window.id());
// }     }

//     pub fn fill_window_with_color(window: &dyn Window, color: u32) {
//         GC.with(|gc| {
//             let size = window.surface_size();
//             let (Some(width), Some(height)) = (NonZeroU32::new(size.width),
// NonZeroU32::new(size.height)) else {                 return;
//             };
...
//     #[allow(dead_code)]
//     pub fn fill_window(window: &dyn Window) { fill_window_with_color(window, 0xff181818); }
